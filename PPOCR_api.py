...
def GetOcrApi(
        exePath: str = os.path.join(os.path.dirname(__file__), "lib", "PaddleOCR-json.exe"),
        modelsPath: str = None,
        argument: dict = None,
        ipcMode: str = "pipe",
):
    """获取识别器API对象。\n
    `exePath`: 识别器`PaddleOCR_json.exe`的路径，默认从当前文件夹的 `lib` 目录查找。\n
    `modelsPath`: 识别库`models`文件夹的路径。若为 None 则默认识别库与识别器在同一目录下。\n
    `argument`: 启动参数，字典`{"键":值}`。参数说明见 https://github.com/hiroi-sora/PaddleOCR-json\n
    `ipcMode`: 进程通信模式，可选值为套接字模式 `socket` 或 管道模式 `pipe`。用法上完全一致。
    """
    # 检查 exePath 是否存在
    if not os.path.exists(exePath):
        raise FileNotFoundError(f"PaddleOCR_json.exe 文件未找到，路径: {exePath}")
    ...
    if ipcMode == "socket":
        ...
        raise ValueError(
            f'ipcMode 可选值为 套接字模式 "socket" 或 管道模式 "pipe"，不允许值: {ipcMode}'
        )
